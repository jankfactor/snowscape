...
/* Amiga 0x16a70: convert the fully subdivided height word before lighting. */
int TerrainCoastalHeight(unsigned short value)
{
    int height = TerrainSignedHeight(value);
    if (height < 0)
        return 0;
    if (height <= 9)
        return 24 * (height + 1);
    return TerrainSignedHeight((unsigned short)(height + 250));
}
