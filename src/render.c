...
static unsigned char verticesToProject[MAPW * MAPW];
...
void RenderTriangle(TRI *tri)
{
    ...
    /* Sum all three 16.16 depths: each bucket spans 4/3 world units
       of average depth, without needing a division by three. */
    k = (i + j + k) >> 18;
    if (k >= MAXDEPTH)
        return;
    ...
    else
    {
        ...
        g_RenderQueue[k] = tri;
        verticesToProject[tri->a] = 1;
        verticesToProject[tri->b] = 1;
        verticesToProject[tri->c] = 1;
    }
}
...
void RenderModel(MAT43 *mv, V3D *eyePos, int yaw)
{
    ...
    for (x = mx - SCANRANGE; x <= mx + SCANRANGE; ++x)
    {
        ...
        {
            i = x + z * MAPW;
            /* Clipping above must finish before overwriting camera space. */
            if (verticesToProject[i])
