...
{
    static const unsigned char range[48] = {
        4, 48, 51, 49, 89, 89, 90, 90, 91, 91, 92, 92, 118, 118, 114, 114,
        32, 32, 32, 32, 35, 35, 40, 40, 68, 68, 68, 112, 112, 115, 115, 113,
        13, 128, 160, 160, 161, 161, 161, 161, 194, 194, 194, 194, 247, 247, 255, 255
    };

    if (height <= 0)
        return 128; /* Dark blue ocean (#000088). */
    /* Colour bands use converted coastal heights in both map and 3D views. */
    if (height > 1850)
        shade += 32;
    else if (!useSand || height > 300)
        shade += 16;
    return range[shade];
}
#else
