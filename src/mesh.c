 ...
 *         an index from 2 through 23, or 0 for flat sea.
 *
...
int GenerateTerrain(const TerrainSource *source, const TerrainZoomPath *path)
{
    ...
    for (j = 0; j < MAPW; ++j)
        for (i = 0; i < MAPW; ++i)
        {
            ...
            /* Flatten sea and raise the coastal band before world scaling.
               Map generation and subdivision retain the raw signed heights. */
            g_Mesh.verts[IX(i, j)].y = ((fix)TerrainCoastalHeight(
            ...
            if ((i + j) & 1)
            {
                ...
                */
                ...
                k = TerrainLightIndex(tlHeight, tlHeight - trHeight,
                                      trHeight - brHeight);
#ifdef PAL_256
                face.flags = TerrainColor(max(tlHeight, max(trHeight, brHeight)), k, 1);
#else
                face.flags = k;
#endif // PAL_256
                ...
                */
                ...
                k = TerrainLightIndex(blHeight, blHeight - brHeight,
                                      tlHeight - blHeight);
#ifdef PAL_256
                face.flags = TerrainColor(max(tlHeight, max(brHeight, blHeight)), k, 1);
#else
                face.flags = k;
#endif // PAL_256
            ...
            else
            {
                ...
                */
                ...
                k = TerrainLightIndex(tlHeight, tlHeight - trHeight,
                                      tlHeight - blHeight);
#ifdef PAL_256
                face.flags = TerrainColor(max(tlHeight, max(trHeight, blHeight)), k, 1);
#else
                face.flags = k;
#endif // PAL_256
                ...
                */
                ...
                k = TerrainLightIndex(brHeight, blHeight - brHeight,
                                      trHeight - brHeight);
#ifdef PAL_256
                face.flags = TerrainColor(max(brHeight, max(blHeight, trHeight)), k, 1);
#else
                face.flags = k;
#endif // PAL_256
