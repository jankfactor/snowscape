...
#define KEY_F2 114
/* Modes 9 and 13 both use four OS units per screen pixel. */
#define MAP_MOUSE_SHIFT 2
...
extern unsigned int ScreenBank;

#ifdef PAL_256
/* Dark-to-light frame rings using the renderer's Mode 13 palette. */
                                    ...
                                    int zoomLevel)
{
    ...
    /* Keep raw relief slopes and use land colours for the map's sand band. */
    return TerrainColor(TerrainCoastalHeight(rawHeight), shade >> 2, 0);
#else
    if (height < 0)
...
static void DrawMapHelp(unsigned char *screen)
{
    draw_text(screen, 208, 8,
              "LEFT CLICK -\nZOOM INTO CURSOR\n\n"
...
static int SelectMapScreenBank(unsigned char **screen)
{
    ...
    ScreenBank = 1; /* Keep the shared bank switcher in sync with OS_Byte 112. */
    in.r[0] = 113;
    in.r[1] = 1;
    error = _kernel_swi(OS_Byte, &in, &out);
    if (error != NULL) return 1;
...
{
    static const unsigned char mouseBounds[9] = {
        1, 0, 0, 0, 0,
        ((SCREEN_W << MAP_MOUSE_SHIFT) - 1) & 0xff,
        ((SCREEN_W << MAP_MOUSE_SHIFT) - 1) >> 8,
        ((SCREEN_H << MAP_MOUSE_SHIFT) - 1) & 0xff,
        ((SCREEN_H << MAP_MOUSE_SHIFT) - 1) >> 8
    };
    HeightCell *grid;
    HeightCell *display;
    ...
    /* OS_Word 21,1 also brings current and buffered positions inside the box.
       https://www.riscos.com/support/developers/prm/vdu.html */
    in.r[0] = 21;
    in.r[1] = (int)mouseBounds;
    error = _kernel_swi(OS_Word, &in, &out);
    if (error != NULL)
        return -1;
    ...
    for (;;)
    {
        ...
        mousePixelX = out.r[0] >> MAP_MOUSE_SHIFT;
        mousePixelY = SCREEN_H - 1 - (out.r[1] >> MAP_MOUSE_SHIFT);
        ...
        {
            /* OS_Mouse measures Y up from the bottom of the screen. */
            if (validZoomOrigin)
            {
        ...
        {
            /* Restore help after erasing a cursor that overlapped the panel. */
            if (mapDirty || (mouseChanged && drawnMouseX + 5 >= 200))
                DrawMapHelp(screen);
            if (boxVisible)
